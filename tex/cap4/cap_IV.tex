...
\subsection{RF01:API - Autenticação}
\label{requisito:autenticacaoApi}
Todos os recursos, somente podem ser acessados por usuários autenticados. Cada requisição deve conter um hash de acesso no cabeçalho que foi disponibilizado para o cliente no momento em que ele realizou a autenticação. Esse hash deve ser válido apenas por 24 horas contadas à partir de sua criação. 

\subsection{RF02:API - Requisições}
\label{requisito:requisicaoApi}
O serviço não deve armazenar o estado da requisição, toda requisição deve ser entendida como isolada, ou seja, deve se comportar como um serviço burro em que toda e qualquer ação, todas as informações necessárias devem ser enviados pelo cliente solicitante. Isso é conhecido como stateless. 
...
Dessa forma, foram definidos os seguintes Requisitos Não-Funcionais do sistema proposto, que podem ser observados na Tabela \ref{tabelaRequisitosNaoFuncionais}.
...
RNF01 & O sistema operacional do sistema & O sistema deverá ser acessado por varios tipos de sistemas operacionais ao mesmo tempo.
\\\hline
RNF02 &	O desenvolvimento do sistema & O sistema deverá ser desenvolvido ele toda em linguagem de programação JavaScript.
\\ \hline
RNF03 &	Objetos JSON & O sistema devera se comunicar com uma API que fornecerá os recursos necessários através de uma arquitetura do tipo RestFull. As respostas deverão ser no formato JSON.
 \\ \hline
RNF04 &	Intregação com outro sistema & O sistema deverá ser feito que possibilite a intregração com outro sistema.
 \\ \hline
RNF05 &	O sistema deverá ter alta disponibilidade & O sistema deverá ter alta disponibilidade, por exemplo 99\% do tempo.
\\ \hline
RNF06 &	Tempo de processamento & O sistema deverá processo N requisições por um determinado tempo.
\\ \hline
RNF07 &	Confiabilidade & O sistema não deverá apresentar aos usuários quaisquer dados de cunho privativo.
\\ \hline
\end{tabular}
}
...
Para ter acesso aos dados,  em seu primeiro acesso, o aplicativo deverá apresentar um formulário em que o usuário deverá informar seu código de usuário, fornecido pelo centro automotivo, e seu número de documento e caso seja um usuário válido, deverá ser redirecionado para a primeira tela do aplicativo. Essa ação deve ser somente necessária no primeiro acesso. Caso não seja um usuário válido, ou tenha digitado alguma informação errada, uma mensagem deve ser apresentada ao usuário.
